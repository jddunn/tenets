...
def examine(
    path: Path = typer.Argument(
        Path("."),
        help="Path to examine (directory or file)"
    ),
    # Analysis options
    deep: bool = typer.Option(
        False,
        "--deep", "-d",
        help="Perform deep analysis with AST parsing"
    ),
    # Output options
    ...
    ownership: bool = typer.Option(
        False,
        "--ownership",
        help="Show code ownership (requires git)"
    ),
    hotspots: bool = typer.Option(
        False,
        "--hotspots",
        help="Show code hotspots (frequently changed files)"
    ),
...
def _display_analysis_table(analysis, quiet: bool = False):
    ...
    # Language breakdown
    lang_str = ", ".join(f"{lang} ({sum(1 for f in analysis.files if f.language == lang)})" 
...
def _display_complexity_table(analysis):
    ...
    complex_files = [
        ...
        if f.complexity and f.complexity.cyclomatic > 10
    ]
...
def _display_ownership_table(git_analysis):
    ...
    sorted_authors = sorted(
        git_analysis.author_stats.items(),
