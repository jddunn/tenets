...
def rank(
    prompt: str = typer.Argument(
        ..., help="Your query or task to rank files against"
    ),
    ...
    top: Optional[int] = typer.Option(
        None, "--top", "-t", help="Show only top N files"
    ),
    ...
    tree_view: bool = typer.Option(
        False, "--tree", help="Show results as directory tree"
    ),
    show_scores: bool = typer.Option(
        True, "--scores/--no-scores", help="Show relevance scores"
    ),
    show_factors: bool = typer.Option(
        False, "--factors", help="Show ranking factor breakdown"
    ),
    ...
    show_stats: bool = typer.Option(
        False, "--stats", help="Show ranking statistics"
    ),
    verbose: bool = typer.Option(
        False, "--verbose", "-v", help="Show detailed debug information"
    ),
    copy: bool = typer.Option(
        False, "--copy", help="Copy file list to clipboard"
    ),
):
    """
    ...
    try:
        ...
        if min_score:
            ranked_files = [f for f in ranked_files if getattr(f, 'relevance_score', 0) >= min_score]
        ...
        output_content = _format_ranked_files(
            ...
            show_path=show_path,
            prompt=prompt,
            stats=ranker.stats if show_stats else None,
        )
        ...
        if output:
            ...
            if format == "markdown" or format == "tree":
        ...
        if show_stats and ranker.stats:
            _show_stats(ranker.stats)
...
def _format_tree(files: List, show_scores: bool, show_factors: bool, show_path: str) -> str:
    ...
    for dir_path in sorted(dirs.keys()):
        ...
        for file in dirs[dir_path]:
            name = Path(file.path).name
            if show_scores:
                score = getattr(file, 'relevance_score', 0.0)
                file_text = f"📄 {name} [{score:.3f}]"
            else:
                file_text = f"📄 {name}"
...
def _format_html(files: List, show_scores: bool, show_factors: bool, prompt: str, tree_view: bool) -> str:
...
def _get_display_path(path: Path, style: str) -> str:
