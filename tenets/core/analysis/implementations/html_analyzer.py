...
class HTMLAnalyzer(LanguageAnalyzer):
            ...
            if href_match:
                imports.append(
                    ImportInfo(
                        ...
                        line=content[:match.start()].count("\n") + 1,
                        type="html_import",
                        is_relative=not href_match.group(1).startswith(("http://", "https://", "//")),
                    )
                )
        
        # Extract preload/prefetch resources
        preload_pattern = r'<link[^>]+rel=["\'](?:preload|prefetch|preconnect|dns-prefetch)["\'][^>]*>'
    ...
    def extract_structure(self, content: str, file_path: Path) -> CodeStructure:
        ...
        structure.aria_labels = len(re.findall(r'aria-label=["\'][^"\']+["\']', content, re.IGNORECASE))
        structure.aria_labelledby = len(re.findall(r'aria-labelledby=["\'][^"\']+["\']', content, re.IGNORECASE))
        structure.aria_describedby = len(re.findall(r'aria-describedby=["\'][^"\']+["\']', content, re.IGNORECASE))
        structure.aria_roles = len(re.findall(r'role=["\'][^"\']+["\']', content, re.IGNORECASE))
        structure.aria_live = len(re.findall(r'aria-live=["\'][^"\']+["\']', content, re.IGNORECASE))
        
        # Count accessibility features
        # Only count non-empty alt attributes
        structure.alt_texts = len(re.findall(r'<img[^>]*alt=["\']\s*[^"\'\s][^"\']*["\'][^>]*>', content, re.IGNORECASE))
        structure.label_for = len(re.findall(r'<label[^>]*for=["\'][^"\']+["\']', content, re.IGNORECASE))
        structure.tabindex = len(re.findall(r'tabindex=["\'][^"\']+["\']', content, re.IGNORECASE))
        
        # Count custom elements
        structure.custom_elements = len(re.findall(r'<[a-z]+-[a-z-]+[^>]*>', content, re.IGNORECASE))
        
        # Count data attributes
        structure.data_attributes = len(re.findall(r'data-[a-z-]+=["\'][^"\']+["\']', content, re.IGNORECASE))
        
        # Count inline scripts and styles
        structure.inline_scripts = len(re.findall(r'<script[^>]*>(?:(?!</script>).)+</script>', content, re.IGNORECASE | re.DOTALL))
        structure.inline_styles = len(re.findall(r'<style[^>]*>(?:(?!</style>).)+</style>', content, re.IGNORECASE | re.DOTALL))
        structure.inline_style_attrs = len(re.findall(r'style=["\'][^"\']+["\']', content, re.IGNORECASE))
        ...
        event_handlers = [
            ...
            "onfocus", "onblur", "ondblclick", "onscroll"
        ]
        structure.inline_event_handlers = 0
        for handler in event_handlers:
            structure.inline_event_handlers += len(re.findall(f'{handler}=["\'][^"\']+["\']', content, re.IGNORECASE))
        
        # Detect lazy loading
        structure.lazy_loading = len(re.findall(r'loading=["\']lazy["\']', content, re.IGNORECASE))
        
        # Detect async/defer scripts
        structure.async_scripts = len(re.findall(r'<script[^>]*async[^>]*>', content, re.IGNORECASE))
        structure.defer_scripts = len(re.findall(r'<script[^>]*defer[^>]*>', content, re.IGNORECASE))
        
        # Check for service worker registration
        structure.has_service_worker = bool(re.search(r'navigator\.serviceWorker\.register', content, re.IGNORECASE))
        
        # Check for web manifest
        structure.has_manifest = bool(re.search(r'<link[^>]*rel=["\']manifest["\']', content, re.IGNORECASE))
    ...
    def calculate_complexity(self, content: str, file_path: Path) -> ComplexityMetrics:
        ...
        metrics.inline_script_count = structure.inline_scripts if hasattr(structure, 'inline_scripts') else 0
        metrics.event_handler_count = structure.inline_event_handlers if hasattr(structure, 'inline_event_handlers') else 0
        
        # Style complexity
        metrics.stylesheet_count = len([l for l in structure.links if l.get('rel') == 'stylesheet']) if hasattr(structure, 'links') else 0
        metrics.inline_style_count = structure.inline_styles if hasattr(structure, 'inline_styles') else 0
        metrics.style_attribute_count = structure.inline_style_attrs if hasattr(structure, 'inline_style_attrs') else 0
        ...
        if hasattr(structure, 'scripts'):
            blocking_scripts = len([s for s in structure.scripts if not s.get('async') and not s.get('defer')])
            metrics.performance_score -= (blocking_scripts * 5)
        
        # Bonus for lazy loading
        if hasattr(structure, 'lazy_loading'):
            metrics.performance_score = min(100, metrics.performance_score + (structure.lazy_loading * 2))
        ...
        metrics.has_integrity_checks = bool(re.search(r'integrity=["\'][^"\']+["\']', content, re.IGNORECASE))
