...
class JavaAnalyzer(LanguageAnalyzer):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if match:
                ...
                imports.append(ImportInfo(
                    ...
                    is_relative=False,
                    category=category,
                    package=module.rsplit('.', 1)[0] if '.' in module and not is_wildcard else module.rstrip('.*')
                ))
    ...
    def extract_exports(self, content: str, file_path: Path) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(method_pattern, content):
            ...
            if method_name not in ['if', 'for', 'while', 'switch', 'catch', 'new', 'return', 'throw']:
    ...
    def calculate_complexity(self, content: str, file_path: Path) -> ComplexityMetrics:
        ...
        decision_keywords = [
            ...
            r'\bdo\b', r'\bswitch\b', r'\bcase\b', r'\bcatch\b', r'\bthrow\b',
            r'\b&&\b', r'\|\|', r'\?'  # Logical operators and ternary
        ]
        ...
        metrics.stream_operations = len(re.findall(r'\.\s*(?:stream|filter|map|reduce|collect|forEach)\s*\(', content))
    ...
    def _extract_methods(self, class_body: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(method_pattern, class_body):
            ...
            if method_name in ['if', 'for', 'while', 'switch', 'new', 'return', 'throw', 'class', 'interface']:
                continue
    ...
    def _extract_fields(self, class_body: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(field_pattern, class_body):
            ...
            fields.append({
                ...
                'is_constant': 'static' in (match.group(2) or '') and 'final' in (match.group(2) or ''),
                'line': class_body[:match.start()].count('\n') + 1
            })
    ...
    def _detect_framework(self, content: str, annotations: List[str]) -> Optional[str]:
        ...
        """
        # Spring Framework
        spring_annotations = {'Controller', 'Service', 'Repository', 'Component', 
                             'Autowired', 'Bean', 'Configuration', 'RestController',
                             'RequestMapping', 'SpringBootApplication'}
        if spring_annotations.intersection(set(annotations)):
            return 'Spring'
        
        # JUnit
        junit_annotations = {'Test', 'Before', 'After', 'BeforeClass', 'AfterClass',
        ...
        # JPA/Hibernate
        jpa_annotations = {'Entity', 'Table', 'Id', 'GeneratedValue', 'Column', 'OneToMany', 'ManyToOne'}
