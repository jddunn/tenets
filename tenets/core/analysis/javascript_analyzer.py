...
class JavaScriptAnalyzer(LanguageAnalyzer):
    ...
    def extract_imports(self, content: str, file_path: Path) -> List[ImportInfo]:
        ...
        es6_combined = re.compile(r'^\s*import\s+(\w+)\s*,\s*\{([^}]+)\}\s*from\s+[\'"`]([^\'"`]+)[\'"`]')
        es6_side_effect = re.compile(r'^\s*import\s+[\'"`]([^\'"`]+)[\'"`]')
        
        # TypeScript type imports
        ts_type_import = re.compile(r'^\s*import\s+type\s+(?:\{([^}]+)\}|(\w+))\s+from\s+[\'"`]([^\'"`]+)[\'"`]')
        
        # CommonJS patterns
        require_pattern = re.compile(r'(?:const|let|var)\s+(\w+)\s*=\s*require\s*\([\'"`]([^\'"`]+)[\'"`]\)')
        require_destructure = re.compile(r'(?:const|let|var)\s+\{([^}]+)\}\s*=\s*require\s*\([\'"`]([^\'"`]+)[\'"`]\)')
    ...
    def extract_exports(self, content: str, file_path: Path) -> List[Dict[str, Any]]:
        ...
        export_named_declaration = re.compile(r'^\s*export\s+(?:const|let|var|function|class|interface|type|enum)\s+(\w+)')
        export_list = re.compile(r'^\s*export\s*\{([^}]+)\}')
        export_from = re.compile(r'^\s*export\s*\{([^}]+)\}\s*from\s+[\'"`]([^\'"`]+)[\'"`]')
        export_all = re.compile(r'^\s*export\s*\*\s*(?:as\s+(\w+)\s+)?from\s+[\'"`]([^\'"`]+)[\'"`]')
        ...
        for i, line in enumerate(lines, 1):
            ...
            if match:
                ...
                export_type = 'function' if 'function' in value else \
                             'class' if 'class' in value else \
                             'object' if '{' in value else 'default'
            ...
            if match:
                name = match.group(1)
                export_type = 'function' if 'function' in line else \
                             'class' if 'class' in line else \
                             'interface' if 'interface' in line else \
                             'type' if 'type' in line else \
                             'enum' if 'enum' in line else 'variable'
    ...
    def extract_structure(self, content: str, file_path: Path) -> CodeStructure:
        ...
        # Function patterns
        function_pattern = re.compile(r'(?:export\s+)?(?:async\s+)?function\s*\*?\s+(\w+)\s*\(([^)]*)\)')
        arrow_function = re.compile(r'(?:export\s+)?(?:const|let|var)\s+(\w+)\s*=\s*(?:async\s+)?\(([^)]*)\)\s*(?::\s*[^=]+)?\s*=>')
        method_pattern = re.compile(r'^\s*(?:async\s+)?(\w+)\s*\(([^)]*)\)\s*(?::\s*[^{]+)?\s*\{')
        
        # Class patterns
        class_pattern = re.compile(r'(?:export\s+)?(?:abstract\s+)?class\s+(\w+)(?:\s+extends\s+(\w+))?(?:\s+implements\s+([\w,\s]+))?')
        ...
        # React patterns
        react_component = re.compile(r'(?:export\s+)?(?:const|function)\s+([A-Z]\w+)\s*[=:].*?(?:React\.|jsx|tsx|<)')
    ...
    def calculate_complexity(self, content: str, file_path: Path) -> ComplexityMetrics:
        ...
        decision_keywords = [
            r'\bif\b', r'\belse\s+if\b', r'\belse\b', r'\bwhile\b', r'\bfor\b',
            r'\bdo\b', r'\bswitch\b', r'\bcase\b', r'\bcatch\b', r'\bfinally\b',
            r'\b\?\s*[^:]+\s*:', r'\|\|', r'&&', r'\?\?'  # Ternary, logical, nullish
        ]
