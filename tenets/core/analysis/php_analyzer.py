...
class PhpAnalyzer(LanguageAnalyzer):
    ...
    language_name = "php"
    file_extensions = [".php", ".phtml", ".php3", ".php4", ".php5", ".php7", ".php8", ".phps", ".inc"]
    ...
    def extract_imports(self, content: str, file_path: Path) -> List[ImportInfo]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            # Use statements
            use_pattern = re.compile(r'^\s*use\s+((?:function|const)\s+)?([\w\\]+)(?:\s+as\s+(\w+))?\s*;')
    ...
    def extract_structure(self, content: str, file_path: Path) -> CodeStructure:
        ...
        superglobals = ['$_GET', '$_POST', '$_SESSION', '$_COOKIE', '$_FILES', '$_SERVER', '$_ENV', '$_REQUEST']
        ...
        # Check for test file
        structure.is_test_file = ('Test.php' in file_path.name or 
                                  'test.php' in file_path.name.lower() or
                                  file_path.parts and 'tests' in file_path.parts)
        
        # Count anonymous functions/closures
        structure.closure_count = len(re.findall(r'function\s*\([^)]*\)\s*(?:use\s*\([^)]*\))?\s*\{', content))
    ...
    def calculate_complexity(self, content: str, file_path: Path) -> ComplexityMetrics:
        ...
        decision_keywords = [
            r'\bif\b', r'\belseif\b', r'\belse\b', r'\bwhile\b', r'\bfor\b',
            r'\bforeach\b', r'\bdo\b', r'\bswitch\b', r'\bcase\b',
            r'\bcatch\b', r'\bfinally\b', r'\b\?\s*[^:]+\s*:', 
            r'\b&&\b', r'\|\|', r'\band\b', r'\bor\b', r'\bxor\b',
        ...
        metrics.superglobal_usage = len(re.findall(r'\$_(?:GET|POST|SESSION|COOKIE|FILES|SERVER|ENV|REQUEST)\[', content))
        metrics.eval_usage = len(re.findall(r'\beval\s*\(', content))
        metrics.dynamic_calls = len(re.findall(r'\$\w+\s*\(', content))  # Variable functions
        
        # Type hint metrics
        metrics.type_hints = len(re.findall(r':\s*(?:\?)?(?:int|string|bool|float|array|object|callable|iterable|mixed|void|self|parent|static|[\w\\]+)', content))
        metrics.nullable_types = len(re.findall(r'\?(?:int|string|bool|float|array|object|callable|iterable|mixed|[\w\\]+)', content))
        ...
        if metrics.code_lines > 0:
            ...
            type_factor = min(1.0, metrics.type_hints / metrics.function_count) if metrics.function_count > 0 else 0
