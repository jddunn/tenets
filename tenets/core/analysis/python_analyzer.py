...
class PythonAnalyzer(LanguageAnalyzer):
    ...
    def _calculate_halstead_metrics(self, tree: ast.AST) -> Dict[str, float]:
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, (ast.Add, ast.Sub, ast.Mult, ast.Div, ast.Mod, ast.Pow,
                                ...
                                ast.NotIn, ast.UAdd, ast.USub, ast.Invert)):
            ...
            # Count operands (variables, constants)
                ...
                value = node.value if isinstance(node, ast.Constant) else (
                    node.n if isinstance(node, ast.Num) else node.s
                )
        ...
        if n1 > 0 and n2 > 0:
            ...
            metrics['volume'] = metrics['length'] * (math.log2(metrics['vocabulary']) if metrics['vocabulary'] > 0 else 0)
    ...
    def _is_property(self, node: ast.FunctionDef) -> bool:
        ...
        for decorator in node.decorator_list:
            ...
            if decorator_name == 'property' or '.setter' in decorator_name or '.getter' in decorator_name:
                return True
        return False
