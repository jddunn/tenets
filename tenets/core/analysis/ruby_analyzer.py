...
class RubyAnalyzer(LanguageAnalyzer):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if match:
                ...
                imports.append(ImportInfo(
                    ...
                    is_gem=not self._is_stdlib_module(module_name) and not module_name.startswith('.')
                ))
                continue
    ...
    def extract_structure(self, content: str, file_path: Path) -> CodeStructure:
        ...
        # Check for test file
        structure.is_test_file = ('_test.rb' in file_path.name or 
                                  '_spec.rb' in file_path.name or
                                  file_path.parts and 'test' in file_path.parts or
                                  file_path.parts and 'spec' in file_path.parts)
    ...
    def calculate_complexity(self, content: str, file_path: Path) -> ComplexityMetrics:
        ...
        decision_keywords = [
            r'\bif\b', r'\bunless\b', r'\belsif\b', r'\belse\b',
            r'\bwhile\b', r'\buntil\b', r'\bfor\b', r'\bcase\b',
            r'\bwhen\b', r'\brescue\b', r'\b&&\b', r'\|\|',
            r'\band\b', r'\bor\b', r'\?.*:'  # Ternary operator
        ]
        ...
        iterator_methods = [r'\.each\b', r'\.map\b', r'\.select\b', r'\.reject\b', 
                          r'\.times\b', r'\.upto\b', r'\.downto\b']
        ...
        metaprogramming_methods = [
            'define_method', 'method_missing', 'const_missing',
    ...
    def _is_stdlib_module(self, module: str) -> bool:
        ...
        stdlib_modules = {
            ...
            'csv', 'date', 'dbm', 'debug', 'delegate', 'digest', 'drb', 'english',
            'erb', 'etc', 'fcntl', 'fiddle', 'fileutils', 'find', 'forwardable',
            ...
            'ostruct', 'pathname', 'pp', 'prettyprint', 'prime', 'pstore', 'psych',
            'pty', 'racc', 'rake', 'rdoc', 'readline', 'reline', 'resolv',
            'resolv-replace', 'rexml', 'rinda', 'ripper', 'rss', 'rubygems',
            'scanf', 'sdbm', 'securerandom', 'set', 'shellwords', 'singleton',
            'socket', 'stringio', 'strscan', 'syslog', 'tempfile', 'time',
            'timeout', 'tmpdir', 'tracer', 'tsort', 'un', 'uri', 'weakref',
            'webrick', 'win32ole', 'yaml', 'zlib'
        }
    ...
    def _extract_block_body(self, content: str, start_pos: int, block_type: str) -> Optional[str]:
        ...
        while pos < len(content) and block_depth > 0:
            ...
            keyword_match = re.search(r'\b(class|module|def|if|unless|while|until|for|case|begin|do|end)\b', remaining)
            ...
            if keyword in ['class', 'module', 'def', 'if', 'unless', 'while', 'until', 'for', 'case', 'begin', 'do']:
                block_depth += 1
            elif keyword == 'end':
                block_depth -= 1
                if block_depth == 0:
        ...
        for i, line in enumerate(lines):
            ...
            if method_match:
                ...
                methods.append({
                    ...
                    'is_bang_method': method_name.endswith('!'),
                    'is_setter': method_name.endswith('='),
                    'is_operator': bool(re.match(r'^[\+\-\*\/\%\[\]<>=!&|]+$', method_name.replace('self.', '')))
                })
    ...
    def _extract_toplevel_methods(self, content: str) -> List[FunctionInfo]:
        ...
        for i, line in enumerate(lines):
            ...
            if in_class_or_module:
                if re.search(r'\b(class|module|def|if|unless|while|until|for|case|begin|do)\b', line):
                    depth += 1
                elif re.search(r'\bend\b', line):
                    depth -= 1
                    if depth == 0:
    ...
    def _detect_framework(self, content: str, file_path: Path) -> Optional[str]:
        ...
        # Sinatra indicators
        if re.search(r'require\s+[\'"]sinatra[\'"]', content) or re.search(r'\b(?:get|post|put|delete|patch)\s+[\'"]/', content):
            return 'Sinatra'
        
        # RSpec indicators
        if re.search(r'require\s+[\'"]spec_helper[\'"]', content) or re.search(r'\bdescribe\s+[\'"]', content):
            return 'RSpec'
        
        # Minitest indicators
        if re.search(r'require\s+[\'"]minitest', content) or re.search(r'class\s+\w+\s*<\s*Minitest::Test', content):
            return 'Minitest'
