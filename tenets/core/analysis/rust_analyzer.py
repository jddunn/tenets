...
class RustAnalyzer(LanguageAnalyzer):
    ...
    def extract_structure(self, content: str, file_path: Path) -> CodeStructure:
        ...
        for match in re.finditer(struct_pattern, content):
            ...
            following_char = content[match.end():match.end()+1] if match.end() < len(content) else ''
            if following_char == '(':
                struct_type = 'tuple'
            elif following_char == ';':
                struct_type = 'unit'
        ...
        for match in re.finditer(macro_pattern, content):
            structure.macros.append({
                'name': match.group(1),
                'line': content[:match.start()].count('\n') + 1,
                'is_exported': '#[macro_export]' in content[max(0, match.start()-100):match.start()]
            })
    ...
    def calculate_complexity(self, content: str, file_path: Path) -> ComplexityMetrics:
        ...
        decision_keywords = [
            ...
            r'\bloop\b', r'\bmatch\b', r'\b=>\b',  # Match arms
            r'\b&&\b', r'\|\|', r'\?'  # Try operator
        ]
        ...
        metrics.unsafe_score = (metrics.unsafe_blocks + metrics.unsafe_functions * 2 + 
                               metrics.unsafe_traits * 3 + metrics.unsafe_impl * 3)
        ...
        metrics.trait_bounds = len(re.findall(r':\s*(?:Send|Sync|Clone|Copy|Debug|Display)', content))
        ...
        if metrics.code_lines > 0:
            ...
            error_handling_factor = min(1.0, metrics.question_marks / 
                                       (metrics.unwrap_calls + metrics.expect_calls + 1))
            ...
            if in_dependencies or in_dev_dependencies:
                ...
                if '=' in line:
                    ...
                    imports.append(ImportInfo(
                        ...
                        type="cargo_dev_dependency" if in_dev_dependencies else "cargo_dependency",
                        is_relative=False,
