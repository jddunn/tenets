...
class TenetInjector:
    ...
    def inject_tenets(
        ...
        sorted_tenets = sorted(
            tenets,
            key=lambda t: (t.priority.weight, t.metrics.reinforcement_needed),
            reverse=True
        )
        ...
        if self.reinforce_at_end and len(sorted_tenets) > 3:
            reinforcement = self._create_reinforcement_section(
                sorted_tenets[:3],  # Top 3 most important
                format
            )
            injected_content += f"\n\n{reinforcement}"
    ...
    def _find_injection_points(
        ...
        if strategy == InjectionPosition.TOP:
            ...
            for i, section in enumerate(structure['sections']):
                ...
                # Avoid code blocks
                in_code = any(
                    cb['start'] <= position <= cb['end']
                    for cb in structure['code_blocks']
                )
                
                if not in_code:
                    candidate_points.append(InjectionPoint(
                        position=position,
                        score=score,
            ...
            for point in candidate_points:
                ...
                too_close = any(
                    abs(point.position - p.position) < self.min_distance_between
                    for p in selected
                )
            ...
            for i in range(tenet_count):
                ...
                if structure['natural_breaks']:
                    distances = [
                        (abs(b - target_position), b)
                        for b in structure['natural_breaks']
                    ]
                    distances.sort()
                ...
                points.append(InjectionPoint(
                    position=best_break,
                    score=0.7,
                    reason=f"distributed_position_{i+1}"
                ))
    ...
    def _create_reinforcement_section(
        self,
        top_tenets: List[Tenet],
        format: str
    ) -> str:
        ...
        if format == "markdown":
            ...
            for tenet in top_tenets:
                section += f'  <principle priority="{tenet.priority.value}">{tenet.content}</principle>\n'
    ...
    def calculate_optimal_injection_count(
        self,
        content_length: int,
        available_tenets: int,
        max_token_increase: int = 1000
    ) -> int:
