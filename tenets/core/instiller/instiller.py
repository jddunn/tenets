...
class Instiller:
    ...
    def __init__(self, config: TenetsConfig):
        ...
        self.injector = TenetInjector(config.tenet_injection_config if hasattr(config, 'tenet_injection_config') else None)
        ...
        self.logger.info("Instiller initialized with %d active tenets", len(self.manager._tenet_cache))
    ...
    def _get_tenets_for_instillation(
        ...
        if force:
            ...
            candidates = [
                t for t in self.manager._tenet_cache.values()
                if t.status != TenetStatus.ARCHIVED
            ]
        else:
        ...
        optimal_count = self.injector.calculate_optimal_injection_count(
            ...
            available_tenets=len(candidates),
    ...
    def export_instillation_history(
        ...
        export_data = {
            ...
            'version': '1.0',
            'instillations': [
                result.to_dict() for result in self._cache.values()
            ],
        ...
        if format == "json":
            ...
            with open(output_path, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['Timestamp', 'Session', 'Tenets Count', 'Token Increase', 'Strategy'])
