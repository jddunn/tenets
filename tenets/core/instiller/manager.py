...
class TenetManager:
    ...
    def _init_database(self) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_sessions_session ON tenet_sessions(session_id)")
    ...
    def add_tenet(
        ...
        tenet = Tenet(
            ...
            category=category if isinstance(category, TenetCategory) else (
                TenetCategory(category) if category else None
            ),
            author=author
        )
    ...
    def get_tenet(self, tenet_id: str) -> Optional[Tenet]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.execute(
                "SELECT data FROM tenets WHERE id LIKE ?",
                (f"{tenet_id}%",)
            )
            row = cursor.fetchone()
    ...
    def create_collection(
        ...
        name: str,
        description: str = "",
        tenet_ids: Optional[List[str]] = None
    ) -> TenetCollection:
    ...
    def analyze_tenet_effectiveness(self) -> Dict[str, Any]:
        ...
        sorted_by_injection = sorted(
            self._tenet_cache.values(),
            key=lambda t: t.metrics.injection_count,
            reverse=True
        )
        stats['most_injected'] = [
            {
