...
class StopwordManager:
    ...
    def _load_default_sets(self):
        ...
        if code_file.exists():
            self._sets['code'] = self._load_set_from_file(
                code_file,
                name='code',
                description='Minimal stopwords for code search'
            )
        else:
        ...
        if prompt_file.exists():
            self._sets['prompt'] = self._load_set_from_file(
                prompt_file,
                name='prompt',
                description='Aggressive stopwords for prompt parsing'
            )
        else:
            ...
            self._sets['prompt'] = StopwordSet(
                ...
                words=self._sets['code'].words | {
                    ...
                    'must', 'can', 'need', 'want', 'please', 'help', 'make', 'create',
                    'implement', 'add', 'get', 'set', 'show', 'find', 'use', 'using'
                },
                description='Aggressive stopwords for prompt parsing (fallback)'
            )
            
    def _load_set_from_file(
        self, 
        file_path: Path,
        name: str,
        description: str
    ...
    def combine_sets(
        ...
        return StopwordSet(
            ...
            description=f"Combined from: {', '.join(sets)}"
        )