...
class PromptParser:
    ...
    def _init_patterns(self):
        ...
        self._temporal_patterns = {
            ...
            'indicators': [
                ...
                'updated', 'last', 'past', 'previous', 'ago'
            ]
        }
    ...
    def _detect_external_reference(self, text: str) -> Optional[ExternalReference]:
        ...
        if 'gitlab' in parsed.netloc:
            match = re.search(
                r'gitlab\.[^/]+/([^/]+)/([^/]+)/(-/)?(?:issues|merge_requests)/(\d+)',
                url
            )
            if match:
                return ExternalReference(
    ...
    def _fetch_external_content(self, ref: ExternalReference) -> str:
        ...
        try:
            if ref.type == 'github':
                ...
                if response.status_code == 200:
                    ...
                    if comments_response.status_code == 200:
                        ...
                        for comment in comments[:5]:  # First 5 comments
                            content_parts.append(f"Comment by {comment.get('user', {}).get('login', 'unknown')}: {comment.get('body', '')}")
                ...
                api_url = f"https://{gitlab_domain}/api/v4/projects/{group}%2F{project}/issues/{number}"
                    ...
                    if notes_response.status_code == 200:
                        ...
                        for note in notes[:5]:
                            if not note.get('system'):  # Skip system notes
                                content_parts.append(f"Comment by {note.get('author', {}).get('name', 'unknown')}: {note.get('body', '')}")
    ...
    def _simple_keyword_extraction(self, text: str) -> List[str]:
        ...
        stopwords = {
            ...
            'please', 'want', 'needs', 'like', 'help', 'use', 'using', 'implement',
            'add', 'create', 'make', 'build', 'get', 'set', 'update', 'delete'
        }
    ...
    def _extract_programming_keywords(self, text: str) -> List[str]:
        ...
        frameworks = [
            ...
            'jest', 'pytest', 'mocha', 'jasmine', 'unittest'
        ]
    ...
    def _extract_file_patterns(self, text: str) -> List[str]:
        ...
        file_mentions = re.findall(
            r'\b(?:file|in|from)\s+([a-zA-Z0-9_\-/]+\.\w{2,4})',
            text
        )
        ...
        dir_patterns = re.findall(
            r'(?:in|from|under)\s+([a-zA-Z0-9_\-/]+/)(?:\s|$)',
            text
        )
    ...
    def _extract_temporal_context(self, text: str) -> Optional[TemporalContext]:
        ...
        has_temporal = any(
            ...
            for indicator in self._temporal_patterns['indicators']
        )
    ...
    def _extract_scope(self, text: str) -> Dict[str, Any]:
        ...
        module_pattern = r'\b(?:in|for|of)\s+(?:the\s+)?([a-z][a-z0-9_]*)\s+(?:module|package|component)'
