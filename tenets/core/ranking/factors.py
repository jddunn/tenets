...
class RankingFactors:
    ...
    def get_top_factors(self, weights: Dict[str, float], n: int = 5) -> List[Tuple[str, float, float]]:
...
class RankedFile:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if not isinstance(other, RankedFile):
            return False
        return (
            self.analysis.path == other.analysis.path and 
            abs(self.score - other.score) < 0.001
        )
    ...
    def explain_ranking(
        ...
        for i, ranked_file in enumerate(ranked_files[:top_n], 1):
            ...
            if include_factors:
                ...
                for factor_name, value, contribution in top_factors:
                    lines.append(f"     - {factor_name:20s}: {value:.3f} × {weights.get(factor_name, 0):.2f} = {contribution:.3f}")
    ...
    def compare_rankings(
        ...
        for path, rank1, rank2, diff in differences[:10]:
            if diff > 0:
                ...
                lines.append(f"{Path(path).name:30s}: #{rank1:3d} → #{rank2:3d} ({direction}{diff:3d})")
