...
class MLConfig:
    ...
    def validate(self):
        ...
        if self.token_limit < self.min_token_limit:
            raise ValueError(f"Token limit {self.token_limit} below minimum {self.min_token_limit}")
    ...
    def get_adaptive_token_limit(
        self, num_files: int, available_memory_mb: Optional[float] = None
    ) -> int:
        """
        ...
        if total_memory_needed > available_memory_mb * 0.5:
            ...
            adjusted_limit = max(self.min_token_limit, min(adjusted_limit, self.max_token_limit))
    ...
    def estimate_processing_time(
        self, num_files: int, avg_file_size: int = 5000
    ) -> Dict[str, float]:
        """
...
def get_optimal_ml_config(num_files: int, performance_priority: str = "balanced") -> MLConfig:
    """
