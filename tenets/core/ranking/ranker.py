...
class RelevanceRanker:
    ...
    def __init__(
        ...
        self.use_stopwords = (
            use_stopwords if use_stopwords is not None 
            else config.ranking.use_stopwords
        )
    ...
    def rank_files(
        ...
        for i, rf in enumerate(ranked_files):
            ...
            if rf.score >= threshold:
                ...
                if explain:
                    rf.explanation = rf.generate_explanation(
                        strategy.get_weights(), verbose=True
                    )
    ...
    def _analyze_corpus(
        ...
        stats['total_imports'] = sum(
            len(importers) for importers in stats['import_graph'].values()
        )
        
        # Build dependency tree (simplified)
        stats['dependency_tree'] = self._build_dependency_tree(
            files, stats['import_graph']
        )
    ...
    def get_ranking_explanation(
        ...
        return explainer.explain_ranking(
            ranked_files[:top_n],
            weights,
            top_n=top_n
        )
