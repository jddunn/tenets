...
from tenets.core.nlp.programming_patterns import ProgrammingPatterns
...
class FastRankingStrategy(RankingStrategy):
    ...
    def _match_with_word_boundaries(
        self, keyword: str, text: str, allow_variations: bool = True
    ) -> MatchResult:
...
class BalancedRankingStrategy(FastRankingStrategy):
    ...
    def rank_file(
        ...
        factors.structure_score = self._calculate_structure_score(file, prompt_context)
    ...
    def _calculate_tfidf_score(
        self, file: FileAnalysis, prompt_context: PromptContext, tfidf_model: TFIDFCalculator
    ) -> float:
    ...
    def _calculate_structure_score(
        ...
        if hasattr(file, "imports") and file.imports:
            for import_info in file.imports:
                ...
                module = (
                    str(import_info.module)
                    if hasattr(import_info, "module") and import_info.module
                    else ""
                )
    ...
    def _calculate_tfidf_score(
        self, file: FileAnalysis, prompt_context: PromptContext, tfidf_model: Any
    ) -> float:
...
class ThoroughRankingStrategy(BalancedRankingStrategy):
    ...
    def __init__(self, config=None):
        ...
        is_testing = (
            ...
            or os.environ.get("TENETS_SUMMARIZER_ENABLE_ML_STRATEGIES") == "false"
        )
    ...
    def _init_ml_components(self):
        ...
        try:
            ...
            self.logger.warning(
                "Continuing without ML features - close other applications to free memory"
            )
            self._embedding_model = None
        except Exception as e:
        ...
        if self._embedding_model:
            ...
            self.logger.warning(
                "ML components not available - install sentence-transformers for semantic features"
            )
    ...
    def rank_file(
        ...
        if not self._enable_ml:
            ...
            factors.dependency_score = self._calculate_dependency_score(file, prompt_context)
            return factors

        self.logger.debug(
            f"ML features enabled, embedding_model={self._embedding_model is not None}, "
        ...
        if self._embedding_model:
            ...
            factors.semantic_similarity = self._calculate_semantic_similarity_optimized(
                file, prompt_context, corpus_stats
            )
        ...
        if self._pattern_matcher:
            factors.pattern_score = self._calculate_pattern_score(file, prompt_context)
        else:
            factors.pattern_score = 0.0
        ...
        try:
            ...
            self.logger.debug(
                f"ThoroughRankingStrategy total: {total_time * 1000:.1f}ms "
        ...
        try:
            ...
            if query_key not in self._query_embedding_cache:
                ...
                self._query_embedding_cache[query_key] = self._embedding_model.encode(
                    query_text, show_progress=False, normalize=True
                )
    ...
    def _calculate_pattern_score(self, file: FileAnalysis, prompt_context: PromptContext) -> float:
    ...
    def _calculate_class_relevance(self, structure: Any, prompt_context: PromptContext) -> float:
    ...
    def _calculate_function_relevance(self, structure: Any, prompt_context: PromptContext) -> float:
...
def create_ranking_strategy(algorithm: str = "balanced") -> RankingStrategy:
    ...
    if algorithm not in strategies:
        raise ValueError(f"Unknown algorithm: {algorithm}. Choose from: {list(strategies.keys())}")
