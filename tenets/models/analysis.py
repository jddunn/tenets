...
class FileAnalysis:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'FileAnalysis':
        ...
        if 'imports' in data:
            data['imports'] = [ImportInfo(**i) if isinstance(i, dict) else i for i in data['imports']]
        if 'classes' in data:
            data['classes'] = [ClassInfo(**c) if isinstance(c, dict) else c for c in data['classes']]
        if 'functions' in data:
            data['functions'] = [FunctionInfo(**f) if isinstance(f, dict) else f for f in data['functions']]
        if 'structure' in data and isinstance(data['structure'], dict):
            ...
            if 'classes' in structure_data:
                structure_data['classes'] = [ClassInfo(**c) if isinstance(c, dict) else c for c in structure_data['classes']]
            if 'functions' in structure_data:
                structure_data['functions'] = [FunctionInfo(**f) if isinstance(f, dict) else f for f in structure_data['functions']]
            if 'imports' in structure_data:
                structure_data['imports'] = [ImportInfo(**i) if isinstance(i, dict) else i for i in structure_data['imports']]
