...
class CouplingGraph(VisualizationBase):
    ...
    def find_clusters(self) -> List[Set[str]]:
        ...
        for coupling in self.couplings:
            if coupling.coupling_type in ["strong", "moderate"]:
                G.add_edge(coupling.file1, coupling.file2, weight=coupling.strength)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        most_coupled = (
            max(file_coupling_counts.items(), key=lambda x: x[1])
            if file_coupling_counts
            else (None, 0)
        )
    ...
    def _render_html(self, width: int, height: int) -> str:
        ...
        edges = [
            (c.file1, c.file2) for c in self.couplings if c.coupling_type in ["strong", "moderate"]
        ]
        ...
        node_trace = go.Scatter(
            ...
            marker=dict(
                ...
                size=node_size,
                color=node_color,
                colorbar=dict(thickness=15, title="Couplings", xanchor="left", titleside="right"),
                line_width=2,
            ),
...
def analyze_coupling_from_git(
    repo_path: Path, since: Optional[datetime] = None, min_coupling: int = 2
) -> CouplingGraph:
