...
class TestContextFormatter:
    ...
    def test_format_markdown(self, formatter, aggregated_data, prompt_context):
        ...
        with patch('tenets.core.distiller.formatter.datetime') as mock_dt:
            ...
            result = formatter._format_markdown(
                aggregated_data,
                prompt_context,
                "test-session"
            )
    ...
    def test_format_xml_escaping(self, formatter, prompt_context):
        ...
        data = {
            ...
            'statistics': {'files_analyzed': 1, 'files_included': 1, 
                         'files_summarized': 0, 'files_skipped': 0,
    ...
    def test_format_git_context_markdown(self, formatter):
        ...
        git_context = {
            'branch': 'feature/auth',
            'recent_commits': [
                {'sha': 'abc123def', 'message': 'Add login', 
                 'author': 'John', 'date': '2024-01-01'},
                {'sha': 'def456ghi', 'message': 'Fix bug',
    ...
    def test_format_git_context_xml(self, formatter):
        ...
        git_context = {
            'branch': 'main',
            'recent_commits': [
                {'sha': 'abc123', 'message': 'Test & fix',
                 'author': 'Dev <dev@test.com>', 'date': '2024-01-01'}
            ]
        }
