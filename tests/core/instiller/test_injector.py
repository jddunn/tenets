...
from tenets.core.instiller.injector import (
    TenetInjector, InjectionPosition, InjectionPoint
)
...
def sample_tenets():
    ...
    tenets = [
        ...
        Tenet(
            content="Handle all exceptions explicitly",
...
class TestTenetInjector:
    ...
    def test_inject_tenets_empty_list(self, injector, markdown_content):
        ...
        content, metadata = injector.inject_tenets(
            markdown_content,
            [],
            format="markdown"
        )
...
<section>Content 2</section>
    ...
    def test_find_injection_points_top(self, injector):
        ...
        points = injector._find_injection_points(
            content="x" * 1000,
            structure=structure,
            strategy=InjectionPosition.TOP,
            tenet_count=3
        )
    ...
    def test_find_injection_points_distributed(self, injector):
        ...
        structure = {
            'sections': [],
            'code_blocks': [],
            'natural_breaks': [100, 300, 500, 700]
        }
    ...
    def test_create_reinforcement_section_markdown(self, injector, sample_tenets):
        ...
        section = injector._create_reinforcement_section(
            sample_tenets[:2],
            "markdown"
        )
    ...
    def test_calculate_optimal_injection_count(self, injector):
        ...
        count = injector.calculate_optimal_injection_count(
            content_length=50000,
            available_tenets=100,
            max_token_increase=100
        )
    ...
    def test_inject_into_context_result(self, injector, sample_tenets):
        ...
        modified = injector.inject_into_context_result(
            context_result,
            sample_tenets
        )
    ...
    def test_injection_with_reinforcement(self, injector):
        ...
        tenets = [
            Tenet(content=f"Principle {i}", priority=Priority.HIGH)
            for i in range(5)
        ]
...
"""
        ...
        modified, metadata = injector.inject_tenets(
            content,
            tenets,
            format="markdown"
        )
