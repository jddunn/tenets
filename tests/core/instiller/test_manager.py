...
def sample_tenets(manager):
    ...
    tenets.append(
        manager.add_tenet(
            "Always use type hints", priority=Priority.HIGH, category=TenetCategory.STYLE
        )
    )
    tenets.append(
        manager.add_tenet(
            "Handle exceptions properly", priority=Priority.CRITICAL, category=TenetCategory.QUALITY
        )
    )
    tenets.append(
        manager.add_tenet(
            "Write tests for new features", priority=Priority.MEDIUM, category=TenetCategory.TESTING
        )
    )
    return tenets
