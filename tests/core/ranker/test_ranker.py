...
class TestRankingFactors:
    ...
    def test_get_weighted_score_with_custom(self):
        ...
        factors = RankingFactors(
            keyword_match=0.5,
            custom_scores={'auth_patterns': 0.8, 'api_patterns': 0.6}
        )
...
""",
    ...
    def test_path_structure_analysis(self, strategy):
        ...
        file = FileAnalysis(
            path="src/api/controllers/auth_controller.py",
            content="",
            language="python"
        )
    ...
    def test_import_centrality_calculation(self, strategy):
        ...
        corpus_stats = {
            'import_graph': {
                'core/base.py': {'auth.py', 'api.py', 'models.py', 'views.py'}
            }
        }
...
class TestThoroughRankingStrategy:
    ...
    def test_semantic_similarity_calculation(self):
        ...
        with patch('tenets.core.ranking.ranker.SentenceTransformer') as mock_st:
            # Setup mock model
            mock_model = Mock()
            mock_model.encode.side_effect = lambda text, **kwargs: Mock(unsqueeze=Mock(return_value=Mock()))
...
""",
    ...
    def test_ast_relevance_analysis(self, strategy):
        ...
        prompt_context = PromptContext(
            text="authentication",
            keywords=["authentication", "authenticate"],
            task_type="feature"
        )
