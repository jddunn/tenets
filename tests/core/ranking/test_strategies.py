...
class TestFastRankingStrategy:
    ...
    def test_hyphen_space_variations(self, strategy):
        ...
        for variant in variations:
            result = strategy._match_with_word_boundaries(variant, content, allow_variations=True)
    ...
    def test_filename_bonus(self, strategy):
        ...
        prompt = PromptContext(text="authentication", keywords=["authentication"])

        score_with = strategy._calculate_keyword_score(file_with_keyword, prompt.keywords)
        score_without = strategy._calculate_keyword_score(file_without_keyword, prompt.keywords)
...
class TestBalancedRankingStrategy:
    ...
    def test_abbreviation_expansion(self, strategy):
        ...
        score_config = strategy._calculate_enhanced_keyword_score(
            FileAnalysis(path="test.py", content=content, language="python", size=100), ["config"]
        )
        assert score_config > 0, "config should match configuration"

        # Test "db" → "database"
        score_db = strategy._calculate_enhanced_keyword_score(
            FileAnalysis(path="test.py", content=content, language="python", size=100), ["db"]
        )
    ...
    def test_plural_singular_normalization(self, strategy):
        ...
        assert strategy._match_plural_singular(
            "entitie", tokens
        ) or strategy._match_plural_singular("entity", tokens)
    ...
    def test_compound_word_matching(self, strategy, camel_case_file):
        ...
        prompt = PromptContext(text="find auth manager", keywords=["auth", "manager"])

        # In balanced mode, should match auth in UserAuthManager
        score = strategy._calculate_enhanced_keyword_score(camel_case_file, prompt.keywords)
...
class TestThoroughRankingStrategy:
    ...
    def test_semantic_features_placeholder(self, strategy):
        ...
        prompt = PromptContext(text="auth system", keywords=["auth", "system"])
    ...
    def test_feature_inheritance_chain(self):
        ...
        for strategy in [fast, balanced, thorough]:
            result = strategy._match_with_word_boundaries(
                "open source", content, allow_variations=True
            )
...
class TestPerformanceCharacteristics:
    ...
    def create_large_file(self, size_kb: int = 100) -> FileAnalysis:
        ...
        return FileAnalysis(
            path="/src/large_file.py", content=content, language="python", size=len(content)
        )
    ...
    def test_relative_performance(self):
        ...
        prompt = PromptContext(
            text="authentication system", keywords=["authentication", "system", "user", "login"]
        )
...
class TestEdgeCases:
    ...
    def test_empty_content(self):
        ...
        empty_file = FileAnalysis(path="/empty.py", content="", language="python", size=0)
    ...
    def test_no_keywords(self):
        ...
        file = FileAnalysis(
            path="/test.py", content="authentication system", language="python", size=100
        )
