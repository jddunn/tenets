...
class TestSessionManager:
    ...
    def test_db_error_handling(self, session_manager_db):
        ...
        with patch.object(session_manager_db._db, 'create_session', side_effect=Exception("DB Error")):
            # Should still work with memory
            session = session_manager_db.create("test_session")
            assert session is not None
            assert session.name == "test_session"
            
        with patch.object(session_manager_db._db, 'list_sessions', side_effect=Exception("DB Error")):
        ...
        with patch.object(session_manager_db._db, 'delete_session', side_effect=Exception("DB Error")):
...
class TestSessionContext:
    ...
    def test_session_context_metadata(self):
        ...
        session = SessionContext(
            session_id="test",
            name="test",
            metadata={"project": "tenets", "version": "1.0"}
        )
