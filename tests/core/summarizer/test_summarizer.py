...
class TestSummarizer:
    ...
    def test_summarize_extractive(self, summarizer):
        ...
        result = summarizer.summarize(text, mode=SummarizationMode.EXTRACTIVE, target_ratio=0.5)
    ...
    def test_summarize_compressive(self, summarizer):
        ...
        result = summarizer.summarize(text, mode=SummarizationMode.COMPRESSIVE, target_ratio=0.5)
    ...
    def test_summarize_with_max_length(self, summarizer):
        ...
        result = summarizer.summarize(
            text, mode=SummarizationMode.EXTRACTIVE, target_ratio=0.5, max_length=100
        )
    ...
    def test_summarize_with_min_length(self, summarizer):
        ...
        result = summarizer.summarize(
            text, mode=SummarizationMode.EXTRACTIVE, target_ratio=0.1, min_length=20
        )
    ...
    def test_summarize_file(self, summarizer, temp_files):
        ...
        result = summarizer.summarize_file(file_analysis, target_ratio=0.5, preserve_structure=True)
    ...
    def test_force_strategy(self, summarizer):
        ...
        result = summarizer.summarize(text, force_strategy=custom_strategy)
    ...
    def test_error_handling(self, summarizer):
        ...
        with patch.object(
            summarizer.strategies[SummarizationMode.EXTRACTIVE], "summarize"
        ) as mock_summarize:
...
class TestBatchSummarization:
    ...
    def test_batch_summarize_texts(self, summarizer):
        ...
        texts = ["First text content.", "Second text content.", "Third text content."]
    ...
    def test_batch_summarize_files(self, summarizer):
        ...
        files = [
            FileAnalysis(path=f"file{i}.txt", content=f"Content for file {i}", size=20, lines=1)
            for i in range(3)
        ]
...
'''
        ...
        result = summarizer.summarize_file(file_analysis, target_ratio=0.3, preserve_structure=True)
