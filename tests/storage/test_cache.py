...
from tenets.storage.cache import (
    MemoryCache, DiskCache, AnalysisCache, CacheManager
)
...
class TestDiskCache:
    ...
    def test_metadata_storage(self, temp_cache_dir):
        ...
        with sqlite3.connect(cache.db_path) as conn:
            cursor = conn.execute(
                "SELECT metadata FROM cache WHERE key = ?",
                ("key_with_meta",)
            )
            row = cursor.fetchone()
    ...
    def test_access_time_update(self, temp_cache_dir):
        ...
        with sqlite3.connect(cache.db_path) as conn:
            cursor = conn.execute(
                "SELECT accessed_at FROM cache WHERE key = ?",
                ("key",)
            )
