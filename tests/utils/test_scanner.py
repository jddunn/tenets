...
class TestFileScanner:
    ...
    def test_include_patterns(self, scanner, test_project):
        ...
        files = scanner.scan([test_project], include_patterns=["*.py"])
    ...
    def test_exclude_patterns(self, scanner, test_project):
        ...
        files = scanner.scan([test_project], exclude_patterns=["test_*.py", "*.json"])
    ...
    def test_max_file_size(self, scanner, test_project):
        ...
        files = scanner.scan([test_project], max_file_size=100)
    ...
    def test_gitignore_respect(self, scanner, test_project):
        ...
        files = scanner.scan([test_project], respect_gitignore=True)
    ...
    def test_symlinks(self, scanner, test_project):
        ...
        files_no_follow = scanner.scan([test_project], follow_symlinks=False)

        # With following symlinks
        files_follow = scanner.scan([test_project], follow_symlinks=True)
    ...
    def test_find_files_by_name(self, scanner, test_project):
        ...
        files = scanner.find_files_by_name(test_project, "main.*", case_sensitive=True)

        assert len(files) == 1
        assert files[0].name == "main.py"

        # Case-insensitive search
        files = scanner.find_files_by_name(test_project, "MAIN.*", case_sensitive=False)

        assert len(files) == 1
        assert files[0].name == "main.py"

        # Wildcard pattern
        files = scanner.find_files_by_name(test_project, "test_*.py")
    ...
    def test_find_files_by_content(self, scanner, test_project):
        ...
        files = scanner.find_files_by_content(test_project, "pytest", case_sensitive=True)

        assert len(files) == 1
        assert files[0].name == "test_main.py"

        # Case-insensitive content search
        files = scanner.find_files_by_content(test_project, "PYTEST", case_sensitive=False)

        assert len(files) == 1
        assert files[0].name == "test_main.py"

        # Search with file pattern filter
        files = scanner.find_files_by_content(test_project, "print", file_patterns=["*.py"])
    ...
    def test_duplicate_removal(self, scanner, test_project):
        ...
        files = scanner.scan(
            [test_project, test_project / "src", test_project / "src"]  # Duplicate
        )
