...
class TestGraphGenerator:
    ...
    def test_process_graph_basic(self, generator, sample_dependency_graph):
        ...
        assert any(
            e["source"] == "main.py" and e["target"] == "utils.py" for e in processed["edges"]
        )
    ...
    def test_process_graph_with_max_nodes(self, generator, sample_dependency_graph):
        ...
        processed = generator._process_graph(sample_dependency_graph, max_nodes=3)
    ...
    def test_cluster_nodes(self, generator, sample_dependency_graph, sample_project_info):
        ...
        clustered = generator._cluster_nodes(
            sample_dependency_graph, "directory", sample_project_info
        )
    ...
    def test_get_cluster(self, generator):
        ...
        # Package clustering
        assert (
            generator._get_cluster("mypackage/submodule/file.py", "package", project_info)
            == "mypackage"
        )

    @patch("tenets.viz.graph_generator.GraphGenerator._generate_graphviz_image")
    def test_generate_image_fallback(
        self, mock_graphviz, generator, sample_dependency_graph, tmp_path
    ):
